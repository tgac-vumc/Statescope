...
def VarQ_C(Nu, Beta, Omega, Ngene, Ncell, Nsample):
    """
    Numerically-stable, memory-safe variance of Q(Y).
    Shapes:
    ...
    Returns:
      (Ngene, Nsample)
    """
    # Use global guards if present; otherwise fall back to safe defaults
    ...
    # ----- VarTerm (same algebra; guard exponentials) -----
    # exp(2*Nu + 2*Omega^2) and exp(2*Nu + Omega^2)
    ...
    VarTerm = (
        exp2  * (VarB.unsqueeze(1) + Bt.unsqueeze(1).square())   # (S,G,C)
        - exp2h * Bt.unsqueeze(1).square()
    ).sum(dim=-1).T                                              # (G,S)

    # ----- CovTerm without forming CovX (key memory win) -----
    # v = E[X] = exp(Nu + 0.5*Omega^2)  -> (S,G,C)
    argv = torch.clamp(Nu + 0.5 * Omega.unsqueeze(0).square(), max=EXP_MAX_)
    v    = torch.exp(argv)                                       # (S,G,C)

    # Σ_l bt_l v_l  and  Σ_l bt_l^2 v_l^2  -> both (S,G)
    sum_bt_v   = torch.einsum('sc,sgc->sg', Bt, v)
    sum_bt2_v2 = torch.einsum('sc,sgc->sg', Bt.square(), v.square())

    # Σ_{l≠k} v_l v_k * CovB_{l,k}  where CovB_{l,k} = - bt_l bt_k / (1+B0)
    CovTerm_SG = - (sum_bt_v.square() - sum_bt2_v2) / (1.0 + B0).unsqueeze(1)  # (S,G)

    return VarTerm + CovTerm_SG.T  # (G,S)
...
def grad_Nu_C(Y, SigmaY, Nu, Omega, Beta, Mu0, Alpha0, Beta0, Kappa0,
    ...
    # v = E[X] = exp(Nu + 0.5*Omega^2)  with overflow guard
    ...
    scaling = 0.5 / SigmaY.square().unsqueeze(1).clamp_min(EPS)  # (G,1,S)
    grad_PY = -(scaling * (a + b)).permute(2, 0, 1)              # (S,G,C)
...
def grad_Omega_C(Y, SigmaY, Nu, Omega, Beta, Mu0, Alpha0, Beta0, Kappa0,
    ...
    # CovTerm without forming CovX, then multiply by Ω_l
    v = ExpX.permute(1, 0, 2)                                             # (G,S,C)
    t = torch.einsum('gsk,skc->gsc', v, CovB)                             # (G,S,C)
    u = t - diagB.unsqueeze(0) * v                                        # (G,S,C)
    g_Var += (2.0 * v * u).permute(0, 2, 1) * Omega.unsqueeze(2)          # (G,C,S)

    # a, b, grad_PY
    a = (g_Var - 2.0 * g_Exp * Var.unsqueeze(1) / Exp.unsqueeze(1)) / (Exp.square().unsqueeze(1))
    Diff = Y - torch.log(Exp) - Var / (2.0 * Exp.square())
    b = -Diff.unsqueeze(1) * (2.0 * g_Exp / Exp.unsqueeze(1) + a)
    grad_PY = torch.sum(-0.5 / SigmaY.square().unsqueeze(1).clamp_min(EPS) * (a + b), dim=2)  # (G,C)
