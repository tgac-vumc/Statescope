...
class BLADE:
    ...
    def grad_Beta(self, Nu, Omega, Beta):
        ...
        assert Beta.data_ptr() == self.Beta.data_ptr()
        # 2. Compute grad_PY
    ...
    def Optimize(
            ...
            method: str = "lbfgs",
            grad_clip: float = 1e4,
            amp_grads: bool = False,
            **opt_kwargs,
        ):

        # -------- AMP for GRADIENTS ONLY (not for E_step) --------
        def _amp_autocast_grad(enabled: bool = False, dtype=torch.bfloat16):
...
def Iterative_Optimization(
    ...
    *,
    ...
    # Create BLADE object
    obj = BLADE(
        logY, SigmaY, Mu0, Alpha, Alpha0, Beta0,
        Kappa0, Nu_Init, Omega_Init, Beta_Init,
        device=device,
        lambda_F=0.0,   # start from 0
    )
    ...
    for i in range(1, iter):
        # lambda annealing: 0 -> 1 with step size 0.0001
        lam = min(i * 0.0001, 1.0)
        obj.lambda_F = float(lam)
        ...
        print(f"[Rep {Rep}] outer iter {i} lambda_F = {obj.lambda_F:.4f} ELBO = {obj_val:.6e}")
    ...
    print("[Deconvolution]", Rep, ": Optimization finished after", len(obj_func), "iterations.")
